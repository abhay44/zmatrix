...
'''
    author : Abhay Gurrala
    email : abhaygurrala444@gmail.com 
'''

'''
A module for converting pdbqt(or)pdb to zmatrices
'''
...
class Converter:
    ...
    def read_cartesian(self, input_file='cartesian.dat'):
        ...
        with open(input_file, 'r') as f:
            ...
            for line in f.readlines():
                if "HETATM" in line:
                    coords = line.split()
                    name = ''.join(filter(str.isalpha, coords[2]))
                    position = []
                    for i in coords[5:8]:
    ...
    def add_atom_to_zmatrix(self, i, line):
        ...
        if np.dot(np.cross(plane1, plane2), r_u) < 0:
            dihedral = -dihedral
        if np.degrees(dihedral) < 10 and np.degrees(dihedral) > -10:
            dihedral = 0
        coords = [[0, distance], [1, np.degrees(angle)], [
            2, np.degrees(dihedral)]]
        
        self.dihedral_matrix.append("d" + str(i))
        self.dihedral_matrix.append(np.degrees(dihedral))
    ...
    def output_values_zmatrix(self, output_file='zmatrix.dat'):
        '''Output the values of the zmatrix to the file'''
            ...
            for line in self.zmatrix:
                ...
                for i in position:
                    for j in range(0, len(i), 2):
                        if str(i[j]) == "0":
                            f.write('r' + str(var) + '=' + str(i[j + 1]) + "\n")
                        elif str(i[j]) == "1":
                            f.write('a' + str(var) + '=' + str(i[j + 1]) + "\n")
                        elif str(i[j]) == "2":
                            # f.write('d' + str(var) + '=' + str(i[j + 1]) + "\n")
                            index = self.dihedral_matrix.index(str('d' + str(var)))
                            if self.dihedral_matrix.count(self.dihedral_matrix[index + 1]) > 1:
                                for i in range(len(self.dihedral_matrix)):
                                    if value:                                     
                                        if self.dihedral_matrix[i] == self.dihedral_matrix[index + 1]:
                                            f.write(str(self.dihedral_matrix[i-1]) + "=" + str(self.dihedral_matrix[i]) + "\n")
                                            value = False
                            else:
                                f.write('d' + str(var) + "=" + str(i[j + 1]) + "\n")
                var += 1
    ...
    def print_variables_zmatrix(self, output_file='zmatrix.dat'):
        ''' Output the variables of zmatrix to the file'''
        k = 0
        with open(output_file, 'w') as f:
            for line in self.zmatrix:
                    ...
                    if k == 1:
                        ...
                        f.write('\t' + '1' + '\t' + 'r' + str(k) + '\t' + '2' + '\t' + 'a' + str(k))
                    elif k >= 3:
                        f.write('\t' + '1' + '\t' + 'r' + str(k) + '\t' + '2' + '\t' + 'a' + str(k) + '\t' + '3' + '\t')
                        index = self.dihedral_matrix.index(str('d' + str(k)))
                        if self.dihedral_matrix.count(self.dihedral_matrix[index + 1]) > 1:
                            for i in range(len(self.dihedral_matrix)):
                                if self.dihedral_matrix[i] == self.dihedral_matrix[index + 1]:
                                    f.write(str(self.dihedral_matrix[i-1]))
                                    break
                        else:
                            f.write('d' + str(k))
                    f.write('\n')
    ...
    def run_cartesian(self, input_file='cartesian.dat', output_file='zmatrix.dat'):
        ...
        self.print_variables_zmatrix(output_file)
        self.output_values_zmatrix(output_file)
